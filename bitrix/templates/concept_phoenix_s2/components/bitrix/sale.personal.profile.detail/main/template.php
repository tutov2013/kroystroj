...
if(strlen($arResult["ID"])>0)
{
	...
	<div class="row">
		...
		<div class="col-xl-5 col-md-6 col-12">
			...
			<form method="post"  class="form profile-detail" action="<?=POST_FORM_ACTION_URI?>" enctype="multipart/form-data">
				<?=bitrix_sessid_post()?>
				...
				<div class="row no-margin">
					...
					<div class="col-12">
						...
						<div class="title-form main1">
				            <?= Loc::getMessage('SPPD_PROFILE_NO', array("#ID#" => $arResult["ID"]))?>
				        </div>
						
						<div class="input">
							<?=Loc::getMessage('SALE_PERS_TYPE')?>:&nbsp;&nbsp;<?=$arResult["PERSON_TYPE"]["NAME"]?>
						</div>
						

						<div class="input <?=($arResult["NAME"]) ? "in-focus" : "";?>">
				            <div class="bg"></div>
				            <span class="desc"><?=Loc::getMessage('SALE_PNAME')?></span>
				            <input class='focus-anim require' name="NAME" type="text" value = "<?=$arResult["NAME"]?>" />
				        </div>
						...
						<?
							...
							if(!empty($arResult["ORDER_PROPS"]))
							{
								foreach($arResult["ORDER_PROPS"] as $block)
								{
									...
									{
										?>
											<div class="simple-title font-md bold"><?= $block["NAME"]?></div>
										<?
										foreach($block["PROPS"] as $property)
										{
											...
											?>
				                                        ...
				                                        <li>
				                                            <label class="input-checkbox-css">
				                                                ...
				                                                <input 
				                                                		...
				                                                		type="checkbox" 
				                                                		value="Y"
				                                                		<?if ($currentValue == "Y" || !isset($currentValue) && $property["DEFAULT_VALUE"] == "Y") echo " checked";?>/><span></span><span class="text"><?=$property["NAME"]?></span>
												...
												elseif ($property["TYPE"] == "TEXT")
												{
													...
													else
													{
														?>
				                                            ...
				                                            <div class="input <?=(strlen($currentValue))? "in-focus":""?>">
				                                                <div class="bg"></div>
				                                                <span class="desc"><?=$property["NAME"]?></span>
				                                                <input 
				                                                		id="sppd-property-<?=$key?>" 
				                                                		class='focus-anim <?=($property["REQUIED"] == "Y") ?"require":""?>' 
				                                                		name="<?=$name?>" 
				                                                		type="text"
				                                                		value="<?=$currentValue?>"/>
				                                            </div>
												...
												{
													?>
													<div class="desc-input-under"><?= $property["DESCRIPTION"] ?></div>
													<?
												}
												?>
						...
						<div class="input">
							...
							<input type="submit" class="button-def main-color active <?=$PHOENIX_TEMPLATE_ARRAY["ITEMS"]["DESIGN"]["ITEMS"]['BTN_VIEW']['VALUE']?>" name="save" value="<?echo GetMessage("SALE_SAVE") ?>">

						</div>

						<?/*<div class="input">
						
							<input type="submit" class="button-def main-color active <?=$PHOENIX_TEMPLATE_ARRAY["ITEMS"]["DESIGN"]["ITEMS"]['BTN_VIEW']['VALUE']?>"  name="apply" value="<?=GetMessage("SALE_APPLY")?>">

						</div>*/?>

						<div class="input">

							<input type="submit" class="button-def main-color active <?=$PHOENIX_TEMPLATE_ARRAY["ITEMS"]["DESIGN"]["ITEMS"]['BTN_VIEW']['VALUE']?>"  name="reset" value="<?echo GetMessage("SALE_RESET")?>">
