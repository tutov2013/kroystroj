...
(function() {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		init: function(parameters)
		{
			...
			this.minSum = BX(parameters.minSumId);
			this.minSumValue = null;

			if(this.minSum)
				this.minSumValue = parseFloat(this.minSum.value);
			...
			this.showZip = parameters.params.SHOW_ZIP_INPUT;
			...
			else
			{
				BX.ajax({
					...
					onsuccess: BX.delegate(function(result) {
						/*if (result.redirect && result.redirect.length && actionData != "noRedirect")
						...
						if(result.success != 'N')
						{
					        ...
					        if($("#btnOrderSaveFly").length>0)
							{
								if($("#btnOrderSaveFly").hasClass('first-click'))
								{
								    $(window).scroll(function() {
								        if($(document).scrollTop() > $("#bx-soa-order").offset().top && $("#btnOrderSaveFly").hasClass('first-click'))
								            $("#btnOrderSaveFly").removeClass('first-click');
								    });
								}
							}
		...
		showPagination: function(entity, node)
		{
			...
			if (pagination.pages.length > 1)
			{
				...
				paginationNode = BX.create('DIV', {
					props: {className: 'bx-pagination cpt-pagination col-12'},
					children: [
						BX.create('DIV', {
		...
		bindEvents: function()
		{
			...
			BX.bind(BX("btnOrderSaveFly"), 'click', BX.proxy(this.clickOrderSaveAction, this));
			BX.bind(BX("btnOrderSave"), 'click', BX.proxy(this.clickOrderSaveAction, this));
			...
			for (i in this.locations)
			{
				...
				if (clearButton)
				{
					BX.bind(clearButton, 'click', function(e){
						var target = e.target || e.srcElement,
							parent = BX.findParent(target, {tagName: 'DIV', className: 'bx-form-group'}),
							locationInput;
		...
		fixLocationsStyle: function(section, hiddenSection)
		{
			...
			if (locationSearchInputs.length)
				for (i = 0; i < locationSearchInputs.length; i++)
					BX.addClass(locationSearchInputs[i], 'bx-form-control');

			if (locationStepInputs.length)
				for (i = 0; i < locationStepInputs.length; i++)
					BX.addClass(locationStepInputs[i], 'bx-form-control');
		},
		...
		clickOrderSaveAction: function(event)
		{
			...
			if($(event.target).hasClass('first-click'))
			{
				$(event.target).removeClass('first-click');
				this.animateScrollTo(this.orderBlockNode, 800, 80);
			}
			else
			{
				if (this.isValidForm())
		...
		editActiveBasketBlock: function(activeNodeMode)
		{
			...
			{
				basketContent = node.querySelector('.bx-soa-section-content-node');
		...
		createBasketItem: function(basketItemsNode, item, index, active)
		{
			...
			htmlPrice = '<div class="sum-price-product bold">'+item.data.SUM+'</div>';

			if(parseInt(item.data.DISCOUNT_PRICE) > 0)
				htmlPrice += '<div class="sum-oldprice-product">'+item.data.SUM_BASE_FORMATED+'</div>';


			basketItemsNode.appendChild(
				BX.create('DIV', {
					props: {className: 'product order-product row'},
					children: [
						BX.create('DIV', {
							props: {className: 'col-sm-8 col-12'},
							children: [
								BX.create('DIV', {
									props: {className: 'img-name row no-gutters'},
									children: [
										this.createBasketItemImg(item.data),
										this.createBasketItemContent(item.data)
									]
						...
						BX.create('DIV', {
							props: {className: 'col-sm-2 col-5 wrapper-quantity'},
							html: item.data.QUANTITY + '&nbsp;' + item.data.MEASURE_TEXT

						}),
						BX.create('DIV', {
							props: {className: 'col-sm-2 col-5 wrapper-sum'},
							html: htmlPrice
		...
		createBasketItemImg: function(data)
		{
			...
			return BX.create('DIV', {
				props: {className: 'img col-3'},
				html: "<a class=\"link_style d-block\" target=\"_blank\" href=\""+globalProductsInfo[data.PRODUCT_ID].DETAIL_URL+"\"><img src=\""+globalProductsInfo[data.PRODUCT_ID].PREVIEW_PICTURE_SRC+"\" alt=\""+globalProductsInfo[data.PRODUCT_ID].NAME+"\"></a>"
			});
		},
		...
		createBasketItemContent: function(data)
		{
			...
			if (data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length)
				titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '" class="d-block bold product-name">' + titleHtml + '</a>';
			...
			return BX.create('DIV', {
				props: {className: 'wr-name col-9 align-self-center'},
				html: globalProductsInfo[data.PRODUCT_ID].HTML
			});
		},
		...
		editCoupons: function(basketItemsNode)
		{
			...
			{
				if(!$.isEmptyObject(basketPoolQuantity))
					recalcBasketAjax({"action": "stopRefreshOrder"});
			}
			...
			if(couponsList.length>0)
			{
				...
				{
					couponCreate(couponsList[i], true);
				}
			}
		},
		...
		getCouponsList: function(active)
		{
			...
			if(this.result.COUPON_LIST.length>0)
			{
		...
		addCoupon: function(coupon)
		{
			...
			var couponListNodes = $(".coupons_block").find(".coupon-one"),
				addCoupon = true;

			if(couponListNodes.length>0)
			{
			...
			if(addCoupon)
				couponCreate(this.getCouponNode({text: coupon}, true), true);

		},

		removeCoupon: function(coupon)
		{
			$(".remove-order-coupon[data-coupon='"+BX.util.htmlspecialchars(coupon)+"']").parents(".coupon-one").remove();
		},
		...
		editActiveRegionBlock: function(activeNodeMode)
		{
			...
			{
				regionContent = node.querySelector('.bx-soa-section-content-node');
				...
				//regionNode.appendChild(regionNode);
		...
		getDeliveryLocationInput: function(node)
		{
			...
			if (location && location[0] && location[0].output)
			{
				...
				labelHtml = '<div class="section-title">'
					+ '<span class="text">'+BX.message('DELIVERY_LOCATION_TITLE') + '</span>'
					+ (currentProperty.REQUIRED == 'Y' ? '<span class="bx-authform-starrequired"> *</span> ' : '')
					+ (currentProperty.DESCRIPTION.length ? ' (' + BX.util.htmlspecialchars(currentProperty.DESCRIPTION) + ')' : '')
					+ '</div>';
				...
				insertedLoc = BX.create('DIV', {
					...
					props: {className: 'bx-form-group bx-soa-location-input-container search-line-location'},
			...
			/*if (location && location[0])
			{
				node.appendChild(
					BX.create('DIV', {
						props: {className: 'bx-soa-reference wr-row'},
			...
			if (altProperty)
			{
				altNode = BX.create('DIV', {
					...
					props: {className: "bx-form-group bx-soa-location-input-container"}
				});
				...
				label = BX.create('DIV', {
					...
					props: {
						className: "input-simple inp-small "+(altProperty.VALUE[0]!="" ? "in-focus": "")
					},
					...
					children:[
						...
						BX.create('INPUT', {
							props: {
								...
								type: 'text',
								autocomplete: 'city',
								className: 'bx-form-control bx-soa-customer-input bx-ios-fix focus-anim bord-1 b-ra '+(altProperty.REQUIRED == 'Y' ? "inp-require": ""),
				...
				input = BX.create('INPUT', {
					props: {
						...
						autocomplete: 'city',
						className: 'bx-form-control bx-soa-customer-input bx-ios-fix',
						name: 'ORDER_PROP_' + altProperty.ID,
						value: altProperty.VALUE
					}
				});*/
		...
		getZipLocationInput: function(node)
		{
			...
			if (zipProperty)
			{
				...
				if(zipProperty.DESCRIPTION.length>0)
				{
					desc = BX.create('DIV', {
						props: 
						{
							className: "soa-description align-self-center col-md-6 col-12"
						},
						html: zipProperty.DESCRIPTION
					}
					);
				}
				...
				propsItemNode = BX.create('DIV', {
					props: 
					{
						className: "bx-form-group bx-soa-location-input-container row " + (this.showZip == "Y" ?"":"d-none")
					},
					children:[
						...
						BX.create('DIV', {
							...
							children:[
								...
								BX.create('DIV', {
									...
									props: {
										className: "input-simple no-mg inp-small "+(zipProperty.VALUE ? "in-focus": "")
									},
									...
									children:[
										...
										BX.create('INPUT', {
											props: {
												...
												type: 'text',
												autocomplete: 'zip',
												className: 'focus-anim bord-1 b-ra '+(zipProperty.REQUIRED == 'Y' ? "inp-require": ""),
		...
		getPersonTypeControl: function(node)
		{
			...
			var profilesLength = BX.util.object_keys(this.result.USER_PROFILES).length,
				personTypesCount = this.result.PERSON_TYPE.length,
				currentType, oldPersonTypeId, i, inputTypes, inputProfiles,
				input, options = [], label, delimiter = false;




			if (personTypesCount > 0)
			{

				inputTypes = BX.create('DIV', {
					props: {className: 'row'}
				});

				inputProfiles = BX.create('DIV', {
						props: {className: 'profiles col-12' + (profilesLength > 0 ? ' col-md-4' : ' d-none')},
					});

				input = BX.create('DIV', {
					props: {
						className: 'wr-row row align-items-center'
					},
					children: [
						BX.create('DIV', {
							props: {className: 'col-12' + (profilesLength > 0 ? ' col-md-8':'') + (personTypesCount == 1 ? ' d-none':'')},
							children: [inputTypes]
						}),
						inputProfiles,
					]
				...
				for (i in this.result.PERSON_TYPE)
				{
					if (this.result.PERSON_TYPE.hasOwnProperty(i))
					{
						...
						label = BX.create('LABEL', {
							props: {
								className: 'input-radio-css green-check'
							},
							children: [
								BX.create('INPUT', {
						...
						node.appendChild(BX.create('DIV', {props: {className: 'col-auto'}, children: [label]}));
				...
				this.getProfilesControl(inputProfiles);
			}
			...
			if (profilesLength)
			{
				...
				)
				{
					...
					input = BX.create('SELECT', {
						props: {className: 'bx-form-control simple-select', name: 'PROFILE_ID'},
						children: options,
						events:{
							change: BX.delegate(function(){
					...
					node.appendChild(
						BX.create('DIV', {
							props: {className: 'bx-form-group bx-soa-location-input-container'},
							children: [profileChangeInput, input]
						})
					);
				}
				else
		...
		editActivePaySystemBlock: function(activeNodeMode)
		{
			...
			{
				paySystemContent = node.querySelector('.bx-soa-section-content-node');
				if (!paySystemContent)
				{
		...
		editPaySystemItems: function(paySystemNode)
		{
			...
			var paySystemItemsContainer = BX.create('DIV', {props: {className: 'bx-soa-pp-item-container row-pay-list row'}}),
				paySystemItemNode, i;
		...
		createPaySystemItem: function(item)
		{
			...
			else
			{
				...
				logoNode.setAttribute('style', 'background-image: url(' + logotype + ');');*/
				logotypeSrc = this.defaultPaySystemLogo;
			}
			...
			label = BX.create('LABEL', {
				props: {className: 'bx-soa-pp-company-graf-container pay-item'},
				children: [
					BX.create('INPUT', {
						props: {
					...
					BX.create('DIV', {
						...
						children: [
							...
							BX.create('DIV', {
								props: {
									className: 'wr-img row align-items-center',
								},
								children: [
									BX.create('DIV', {
										props: {
			...
			itemNode = BX.create('DIV', {
				props: {className: 'bx-soa-pp-company wr-pay-item col-md-3 col-6'},
				children: [label],
		...
		getInnerPaySystem: function()
		{
			...
			label = BX.create('LABEL', {
				...
				children: [
					...
					BX.create('SPAN', {
						props: {className: 'text'},
						html: BX.util.htmlspecialchars(BX.message('INNER_PAY_SYSTEM_TITLE'))+" ("+BX.message('INNER_PS_BALANCE_BEFORE')+this.result.CURRENT_BUDGET_FORMATED+")"
					}),
					
				],
				events: {change: BX.proxy(this.selectPaySystem, this)},
		...
		editActiveDeliveryBlock: function(activeNodeMode)
		{
			...
			{
				deliveryContent = node.querySelector('.bx-soa-section-content-node');
				if (!deliveryContent)
				{
		...
		editDeliveryItems: function(deliveryNode)
		{
			...
			var deliveryItemsContainer = BX.create('DIV', {props: {className: 'col-md-6 col-12 bx-soa-pp-item-container order-delivery-list'}}),
				deliveryItemNode, k;
		...
		editDeliveryInfo: function(deliveryNode)
		{
			...
			var deliveryInfoContainer = BX.create('DIV', {props: {className: 'col-md-6 col-12 bx-soa-pp-desc-container'}}),
			...
			title = BX.create('DIV', {props: {className: 'title bold'}, text: name});
			...
			if (currentDelivery.PRICE >= 0)
			{
				price = BX.create('DIV', {
					...
					html: this.params.MESS_PRICE + ': ' + currentDelivery.PRICE_FORMATED/*this.getDeliveryPriceNodes(currentDelivery)*/
				});


				/*var priceDelive = BX.create('LI', {
					children: [
						BX.create('DIV', {
				...
				if (currentDelivery.PERIOD_TEXT && currentDelivery.PERIOD_TEXT.length)
					infoList = BX.create('DIV', {props: {className: 'bx-soa-pp-list-description'}, html: '<span class="bold">'+this.params.MESS_PERIOD + ': </span>'+currentDelivery.PERIOD_TEXT});
			...
			subTitle = BX.create('DIV', {
				...
				children: [
					BX.create('DIV', {props: {className: 'delivery-text'}, html: currentDelivery.DESCRIPTION}),
					currentDelivery.CALCULATE_DESCRIPTION
						? BX.create('DIV', {props: {className: 'delivery-text'}, html: currentDelivery.CALCULATE_DESCRIPTION})
						: null,
			...
			var box1 = BX.create('DIV', {
					props: {className: 'wr-box'},
					children: [
						BX.create('DIV', {
							props: {className: 'wr-section'},
							children: [
								title,
								price,
			...
			deliveryInfoContainer.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-pp-company order-delivery-item-info'},
					children: [box1, box2]
				})
			);
		...
		{
			var extraServices = [],
				i, currentService, serviceNode, serviceName, comment, input, type = "text";
				...
				if (currentService.editControl.indexOf('this.checked') == -1)
				{
					...
					if (input)
					{
						serviceName = BX.create('DIV', {
							...
							html: '<span class="bold">'+BX.util.htmlspecialchars(currentService.name)+'</span>'
							+ (currentService.price ? ' <span class="delivery-price">(' + currentService.priceFormatted + ')</span>' : '')
						});

						if(currentService.description && currentService.description.length)
						{
							comment = BX.create('DIV', {
								props: {
						...
						if(type == "text")
						{
							BX.addClass(input, "bord-1 b-ra");
							serviceNode = BX.create('DIV', {
								props: {className: "bx-form-group input-simple no-mg inp-small no-desc"},
								children:[input]
							});
						...
						{
							BX.addClass(input, 'simple-select bx-form-control');
							serviceNode = input;
						...
						serviceNode = BX.create('DIV', {
							props: {className: 'wr-section'},
							children:[serviceName, serviceNode, comment]
						});
						...
						BX.create('DIV', {
							...
							[
								BX.create('LABEL', {
									...
									html: currentService.editControl + "<span></span>" 
									+ "<span class=\"text\">"
									+ "<span class=\"bold\">"+BX.util.htmlspecialchars(currentService.name)+"</span>"
									+ (currentService.price ? ' <span class=\"delivery-price\">(' + currentService.priceFormatted + ')</span>' : '')
									+ "</span>"
									+ (currentService.description && currentService.description.length
										? '<div class="delivery-comment">' + BX.util.htmlspecialchars(currentService.description) + '</div>'
										: '')
								}
							)]
						});
		...
		createDeliveryItem: function(item)
		{
			...
			var img = BX.create('DIV', {
					props: {
						className: 'img'
					},
				});
			img.setAttribute('style', 'background-image: url(' + logotypeSrc + ');');


			itemNode = BX.create('LABEL', {
				props: {className: 'bx-soa-pp-company order-delivery-item'},
				children: [
					BX.create('INPUT', {
						props: {
					...
					BX.create('DIV', {
						props: {
							className: 'item row no-gutters align-items-center',
						},
						children:[
							BX.create('SPAN', {
								props: {
		...
		activatePickUp: function(deliveryName)
		{
			...
			this.pickUpBlockNode.querySelector('.bx-soa-section-title').innerHTML = BX.util.htmlspecialchars(deliveryName);
		...
		editActivePickUpBlock: function(activeNodeMode)
		{
			...
			{
				pickUpContent = node.querySelector('.bx-soa-section-content-node');
				...
				pickUpContentCol = BX.create('DIV', {props: {className: 'col-md-6 col-12 order-2'}});
		...
		editPickUpList: function(isNew)
		{
			...
			if (!!isNew)
			{
				/*container = this.pickUpHiddenBlockNode.querySelector('.bx_soa_pickup>.col-12');
				if (!container)*/
				...
				container.appendChild(
					BX.create('DIV', {
						props: {className: 'col-md-6 col-12 order-1'},
						children: [
							pickUpList
						]
					})
		...
		getStoreInfoNode: function(currentStore)
		{
			var children = [], item = null, logotype, imgSrc, imgSrcOrig;
			...
			if (currentStore.PHONE)
			{
				children.push(
					BX.create('DIV', {
						props: {className: 'detail-item bold'},
						html: BX.util.htmlspecialchars(currentStore.PHONE)
					})
				);
			}
			...
			if (currentStore.ADDRESS)
			{
				children.push(
					BX.create('DIV', {
						props: {className: 'detail-item'},
						html: BX.util.htmlspecialchars(currentStore.ADDRESS)
					})
				);
			}
			...
			if (this.params.SHOW_STORES_IMAGES === 'Y')
			{
				...
				imgSrc = logotype && logotype.src_1x;
				imgSrcOrig = logotype.src_orig || "";
			}
			...
			if(imgSrc)
			{
				children.push( 
					...
					BX.create('DIV', {
						props: {className: 'detail-item margin-md'},
						children: [
							BX.create('A', {
								props: {
									className: 'd-inline-block cursor-loop'
								},
								attrs: {
									href: imgSrcOrig,
									'data-gallery': 'pickup-'+currentStore.ID
								},
								children: [
									BX.create('IMG', {
										props: {
			...
			children.push(BX.create('DIV', {
				props: {className: "detail-item margin-md wr-btn"},
				children: [
					BX.create('A', {
						props: {className: 'btn btn-md main-color'},
		...
		createPickUpItem: function(currentStore, options)
		{
			...
			storeNode = BX.create('DIV', {
				props: {className: 'bx-soa-pickup-list-item show-hidden-parent', id: 'store-' + currentStore.ID},
				children: [
					...
					BX.create('DIV', {
						props: {className: 'bx-soa-pickup-l-item-name bold'}, 
					...
					BX.create('DIV', {props: {className: 'bx-soa-pickup-l-item-desc'}, text: currentStore.DESCRIPTION}),
					
					/*BX.create('DIV', {
						props: {className: imgClassName},
						children: [
							logoNode,
							BX.create('DIV', {props: {className: 'bx-soa-pickup-l-item-name bold'}, text: currentStore.TITLE}),
						]
					}),*/

					BX.create('DIV', {
						props: {
								className: 'show-hidden-child hidden'
							},
							children: this.getStoreInfoNode(currentStore),
						}
					...
					BX.create('DIV', {
						props: {
								className: 'show-hidden-wrap mode-toogle'
							},
							html: "<div class=\"show-hidden\"><span class=\"bord-bot white\">"
								+ "<span class = \"s\">" +BX.message('ORDER_PICKUP_BTN_DETAIL') + '</span>'
								+ "<span class = \"h\">" +BX.message('ORDER_PICKUP_BTN_DETAIL_HIDE') + '</span>'
								+"</span></div>"
						}
					),
		...
		editPickUpRecommendList: function(geoLocation)
		{
			...
			if (!container)
			{
				container = this.pickUpBlockNode.querySelector('.bx_soa_pickup>.col-12');
			}

			container.appendChild(
				BX.create('DIV', {
					props: {className: 'bx-soa-pickup-subTitle bold'},
		...
		editActivePropsBlock: function(activeNodeMode)
		{
			...
			{
				propsContent = node.querySelector('.bx-soa-section-content-node');
				if (!propsContent)
				{
		...
		editPropsItems: function(propsNode)
		{
			...
			var propsItemsContainer = BX.create('DIV', {props: {className: 'col-12 bx-soa-customer'}}),
				group, property, groupIterator = this.propertyCollection.getGroupIterator(), propsIterator;

			if (!propsItemsContainer)
				propsItemsContainer = this.propsBlockNode.querySelector('.col-12.bx-soa-customer');
		...
		getPropertyRowNode: function(property, propsItemsContainer, disabled)
		{
			...
			{
				BX.addClass(propsItemNode, "bx-form-group bx-soa-customer-field");
		...
		insertLocationProperty: function(property, propsItemNode, disabled)
		{
			...
			if (property.getId() in this.locations)
			{
				...
				{
					propContainer = BX.create('DIV', {props: {className: 'soa-property-container row'}});
					propRow = this.locations[property.getId()];
					for (i = 0; i < propRow.length; i ++)
					{
						currentLocation = propRow[i] ? propRow[i].output : {};
						insertedLoc = BX.create('DIV', {props: {className: 'bx-soa-loc search-line-location'}, html: currentLocation.HTML});
					...
					this.remakeInputs(property.getSettings(), propContainer);
				}
			}
		},
		...
		remakeInputs: function(settings, propContainer){

			var i, inputs, label, inputsHidden, inputsFile;
					...
					for (i = 0; i < inputs.length; i++)
					{
						...
						propContainer.appendChild(
							BX.create('DIV', {
								...
								props: {
										className: "col-md-6 col-12 parent-add-row"
									},
								...
								children:[
									...
									BX.create('DIV', {
										...
										props: {
											className: "input-simple no-mg inp-small"+(settings.VALUE[0] ? " in-focus ": "")
										},
					...
					for (i = 0; i < inputs.length; i++)
					{
						...
						propContainer.appendChild(
							BX.create('DIV', {
								...
								props: {
										className: "col-12 parent-add-row"
									},
								...
								children:[
									...
									BX.create('DIV', {
										...
										props: {
											className: "textarea-simple inp-small no-mg "+(settings.VALUE[0] ? "in-focus": "")
										},
							...
							if(settings.DESCRIPTION.length>0)
							{
								propContainer.appendChild(
									BX.create('DIV', {
										props: 
										{
											className: "soa-description under-input align-self-center col-12"
										},
										html: settings.DESCRIPTION
									})
								);
			...
			else if(settings.TYPE == "Y/N")
			{
				inputs = propContainer.querySelectorAll('input[type=checkbox]');
				inputsHidden = propContainer.querySelectorAll('input[type=hidden]');
			...
			{
				inputs = propContainer.querySelectorAll('.search-line-location');
			...
			else if(settings.TYPE == "ENUM")
			{
				...
				if(inputs)
				{
					for (i = 0; i < inputs.length; i++)
					{
						BX.addClass(inputs[i], "simple-select");
			...
			{
				inputs = propContainer.querySelectorAll('input[type=button]');
				inputsHidden = propContainer.querySelectorAll('input[type=hidden]');
				inputsFile = propContainer.querySelectorAll('input[type=file]');
				var nodeA = propContainer.querySelectorAll('a');
				...
				if(inputs)
				{
					for (i = 0; i < inputs.length; i++)
					{
						...
						BX.removeClass(inputs[i], "btn btn-default btn-sm");
						BX.addClass(inputs[i], "soa-file-btn file-simple b-ra");

						inputs[i].value = settings.NAME;
							...
							BX.create('DIV', {
								...
								children:[
									...
									BX.create('DIV', {
										...
										props: {
											className: "wr-file-input " + (settings.REQUIRED == 'Y' ? "text-require": "")
										},
										children:[inputs[i], nodeA[i], inputsHidden[i],inputsFile[i]]
									}
								)]
							})
						);
			...
			else if(settings.TYPE == "DATE")
			{
				...
				if(inputs)
				{
					for (i = 0; i < inputs.length; i++)
					{
						BX.addClass(inputs[i], "date-group");
		...
		alterProperty: function(settings, propContainer)
		{
			...
			inputs = propContainer.querySelectorAll('select');
			for (i = 0; i < inputs.length; i++)
				BX.addClass(inputs[i], 'bx-form-control');

			inputs = propContainer.querySelectorAll('textarea');
			for (i = 0; i < inputs.length; i++)
			{
				/*inputs[i].placeholder = settings.DESCRIPTION;*/
				BX.addClass(inputs[i], 'bx-form-control bx-ios-fix focus-anim bord-1 b-ra ' +(settings.REQUIRED == 'Y' ? "text-require": ""));
			}
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.delegate(function(e){
					...
					if (textInputs.length)
					{
						...
						BX.addClass(textInputs[textInputs.length - 1], 'bx-form-control bx-soa-customer-input bx-ios-fix');
					...
					if (textAreas.length)
					{
						...
						BX.addClass(textAreas[textAreas.length - 1], 'bx-form-control bx-ios-fix');
					}
		...
		alterDateProperty: function(settings, inputText)
		{
			...
			/*BX.addClass(parentNode, 'soa-input-group');
			addon = BX.create('DIV', {
			...
			/*BX.bind(addon, 'click', BX.delegate(function(e){
				var target = e.target || e.srcElement,
					parentNode = BX.findParent(target, {tagName: 'DIV', className: 'soa-input-group'});
		...
		isValidForm: function()
		{
			...
			var consent = $("#bx-soa-orderSave").find("input.agreecheck");

			if (consent.length>0 && !consent.prop("checked")) {
	            consent.parents("div.wrap-agree").addClass("has-error");

	            formAttentionScroll(consent.parents("div.wrap-agree").offset().top - 140, 
            	"html:not(:animated), body:not(:animated)");

	            return false;
	        }

			validForm($('#bx-soa-order-form'));
			...
			if (propsErrors.length && !navigated)
			{
				if (this.activeSectionId == this.propsBlockNode.id)
				{
					...
					for (i = 0; i < tooltips.length; i++)
					{
						...
						{
							this.animateScrollTo(BX.findParent(tooltips[i], {className: 'bx-form-group bx-soa-customer-field'}), 800, 50);
							break;
						}
					}
				}
		...
		showValidationResult: function(inputs, errors)
		{
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, {tagName: 'DIV', className: 'bx-form-group'}),
			...
			for (i = 0; i < inputs.length; i++)
			{
				inputDiv = BX.findParent(inputs[i], {tagName: 'DIV', className: 'bx-form-group'});
		...
		validateLocation: function(input, arProperty, fieldName)
		{
			...
			var parent = BX.findParent(input, {tagName: 'DIV', className: 'bx-form-group'}),
		...
		{
			var propsMapContainer = BX.create('DIV', {props: {className: 'col-12'}, style: {marginBottom: '0'}}),
		...
		editPropsComment: function(propsNode)
		{
			...
			propsCommentContainer = BX.create('DIV', {props: {className: 'col-12'}});
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					className: 'bx-form-control bx-soa-customer-textarea bx-ios-fix focus-anim bord-1 b-ra',
					name: 'ORDER_DESCRIPTION'
				},
				text: this.result.ORDER_DESCRIPTION ? this.result.ORDER_DESCRIPTION : ''
			});
			div = BX.create('DIV', {
				props: {className: 'bx-form-group bx-soa-customer-field textarea-simple inp-small no-mg '+(this.result.ORDER_DESCRIPTION ? 'in-focus': '')},
				children: [
					BX.create('DIV', {
						props: {className: 'bg'},
					}),
		...
		emptyBasketHTML: function(countProduct){
			if(countProduct <= 0)
			{
				$("#bx-soa-total").find(".group-info").addClass('d-none');
				$("#bx-soa-total").find(".wr-price").addClass('d-none');
				$("#bx-soa-total").find(".wr-coupon-container").addClass('d-none');
				$("#bx-soa-total").find(".basket-buttons").addClass('d-none');

				if(this.minSumValue)
					$("#bx-soa-total").find(".alert-message-min-sum").removeClass('d-none');

				
			}
			
			else{
				$("#bx-soa-total").find(".group-info").removeClass('d-none');
				$("#bx-soa-total").find(".wr-price").removeClass('d-none');
				$("#bx-soa-total").find(".wr-coupon-container").removeClass('d-none');
				$("#bx-soa-total").find(".basket-buttons").removeClass('d-none');

				if(this.minSumValue)
					$("#bx-soa-total").find(".alert-message-min-sum").addClass('d-none');
		...
		editTotalBlock: function()
		{
			...
			this.emptyBasketHTML(countProduct);
			

			total.ORDER_PRICE = parseFloat(total.ORDER_PRICE);
			total.DISCOUNT_PRICE = parseFloat(total.DISCOUNT_PRICE);
			total.ORDER_WEIGHT = parseFloat(total.ORDER_WEIGHT);
			total.DELIVERY_PRICE = parseFloat(total.DELIVERY_PRICE);
			...
			tmpHTML = "<div class=\"item row no-gutters no-margin\">"
						+ "<div class=\"col-auto\"><div class=\"name\">"+BX.message("ORDER_SIDE_SUM_PRODUCTS")+"</div></div>"
						+ "<div class=\"col\"><div class=\"h-100 dotts\"></div></div>"
						+ "<div class=\"col-auto bold\"><div class=\"value\">"+total.PRICE_WITHOUT_DISCOUNT+"</div></div>"
						+ "</div>";
			...
			{
				tmpHTML += "<div class=\"item row no-gutters no-margin\">"
						+ "<div class=\"col-auto\"><div class=\"name\">"+BX.message("ORDER_SIDE_DISCOUNT_SUM_PRODUCTS")+"</div></div>"
						+ "<div class=\"col\"><div class=\"h-100 dotts\"></div></div>"
						+ "<div class=\"col-auto bold\"><div class=\"value txt-green\">&minus;"+total.DISCOUNT_PRICE_FORMATED+"</div></div>"
						+ "</div>";
			}
			...
			if(curDelivery)
			{
				...
				else
				{
					...
					{
						deliveryValue = "<span class=\"bold txt-green\">"+this.params.MESS_PRICE_FREE+"</span>";
					}
					else
					{
						deliveryValue = "<span class=\"bold\">"+total.DELIVERY_PRICE_FORMATED+"</span>";
					}
				...
				tmpHTML += "<div class=\"item row no-gutters no-margin\">"
						+ "<div class=\"col-auto\"><div class=\"name\">"+BX.message("ORDER_SIDE_SUM_DELIVERY")+"</div></div>"
						+ "<div class=\"col\"><div class=\"h-100 dotts\"></div></div>"
						+ "<div class=\"col-auto\"><div class=\"value\">"+deliveryValue+"</div></div>"
						+ "</div>";
			...
			{
				tmpHTML += "<div class=\"item row no-gutters no-margin\">"
						+ "<div class=\"col-auto\"><div class=\"name\">"+BX.message("SOA_SUM_IT")+"</div></div>"
						+ "<div class=\"col\"><div class=\"h-100 dotts\"></div></div>"
						+ "<div class=\"col-auto bold\"><div class=\"value\">"+total.ORDER_TOTAL_PRICE_FORMATED+"</div></div>"
						+ "</div>";

				tmpHTML += "<div class=\"item row no-gutters no-margin\">"
						+ "<div class=\"col-auto\"><div class=\"name\">"+BX.message("SOA_SUM_PAYED")+"</div></div>"
						+ "<div class=\"col\"><div class=\"h-100 dotts\"></div></div>"
						+ "<div class=\"col-auto bold\"><div class=\"value\">"+total.PAYED_FROM_ACCOUNT_FORMATED+"</div></div>"
						+ "</div>";

				$("#bx-soa-total").find(".total-price").html(total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED);

			}
			else
			{
				$("#bx-soa-total").find(".total-price").html(total.ORDER_TOTAL_PRICE_FORMATED);
			}

			tmpNode = $("#bx-soa-total").find(".group-info");

			console.log(tmpNode);

			tmpNode.html(tmpHTML);
			...
			tmpNode = $("#bx-soa-total").find(".pre-desc");
			tmpNode.text("");

			if (this.options.showPayedFromInnerBudget)
			{
				tmpHTML = BX.message("SOA_SUM_LEFT_TO_PAY");
			}
			else
			{
				tmpHTML = BX.message("ORDER_TOTAL_TITLE");
			}
			...
			tmpNode = $("#bx-soa-total").find(".buttons");

			if(this.minSumValue)
			{
				if(this.minSumValue <= total.ORDER_PRICE)
				{
					tmpNode.removeClass('d-none');
					$("#bx-soa-total").find(".alert-message-min-sum").addClass("d-none");
					$("#bx-soa-orderSave").removeClass("d-none");
				}
				
				else
				{
					tmpNode.addClass('d-none');
					$("#bx-soa-total").find(".alert-message-min-sum").removeClass("d-none");
					$("#bx-soa-orderSave").addClass("d-none");
				}
			}
			else
			{
			...
			tmpNode = null;
			if(!this.result.IS_AUTHORIZED && this.result.AUTH.new_user_registration == "N")
			{
				$("#bx-soa-total").find(".basket-buttons").addClass('d-none');
				$(".callFastOrder").addClass('d-none');
			}
