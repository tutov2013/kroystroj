						...
						if(!empty($_GET["review-name"]) && !empty($_GET["review-rating"]) && !empty($_GET["review-text"])){
							...
							if(!empty($curUserID)){
								...
								if(!$res->SelectedRowsCount()){
									...
									$PROP = array(
										"USER_NAME" => defined("BX_UTF") ? htmlspecialcharsbx($_GET["review-name"]) : iconv("UTF-8","windows-1251//IGNORE", htmlspecialcharsbx($_GET["review-name"])),
									...
									$arLoadReviewArray = Array(
										...
										"DETAIL_TEXT"    => defined("BX_UTF") ? htmlspecialcharsbx($_GET["review-text"]) : iconv("UTF-8","windows-1251//IGNORE", htmlspecialcharsbx($_GET["review-text"])),
										"MODIFIED_BY"    => $curUserID,
										"IBLOCK_ID"      => intval($_GET["iblock_id"]),
										"NAME"           => defined("BX_UTF") ? htmlspecialcharsbx($_GET["review-name"]) : iconv("UTF-8","windows-1251//IGNORE", htmlspecialcharsbx($_GET["review-name"]))." (".date($DB->DateFormatToPHP(CSite::GetDateFormat("SHORT")), time()).") uid:".$curUserID,
										"ACTIVE"         => "N",
					...
					if(!empty($_GET["id"])){
						if(!empty($_GET["iblock_id"])){
							...
							if($arReview = $res->GetNext()){
								$arReview["PROPERTY_BAD_REVIEW_VALUE"] = !empty($arReview["PROPERTY_BAD_REVIEW_VALUE"]) ? intval($arReview["PROPERTY_BAD_REVIEW_VALUE"]) : 0;
						...
						if(!empty($_GET["iblock_id"])){
							...
							$res = CIBlockElement::GetList(
								...
								Array(
									"ID",
									"IBLOCK_ID",
									"PROPERTY_GOOD_REVIEW"
								)
							);
							if($arReview = $res->GetNext()){
								$arReview["PROPERTY_GOOD_REVIEW_VALUE"] = !empty($arReview["PROPERTY_GOOD_REVIEW_VALUE"]) ? intval($arReview["PROPERTY_GOOD_REVIEW_VALUE"]) : 0;
								$arReview["PROPERTY_GOOD_REVIEW_VALUE"]++;
								$db_props = CIBlockElement::GetProperty(intval($_GET["iblock_id"]), intval($_GET["id"]), array("sort" => "asc"), Array("CODE" => "USER_ID_VOTE"));
								if($arProps = $db_props->Fetch()){
									$arUsers[$arProps["VALUE"]] = $arProps["VALUE"];
								}
								CIBlockElement::SetPropertyValuesEx(intval($_GET["id"]), intval($_GET["iblock_id"]), array("USER_ID_VOTE" => $arUsers, "GOOD_REVIEW" => $arReview["PROPERTY_GOOD_REVIEW_VALUE"]));
								echo \Bitrix\Main\Web\Json::encode(
									array(
										"ERROR" => "N",
										"SUCCESS" => "Y",
