			...
			<div class="catalog-section-list-item">
				<div class="catalog-section-list-item-wp">
					<?if(!empty($arElement["PICTURE"])):?>
						...
						<a href="<?=$arElement["SECTION_PAGE_URL"]?>" class="catalog-section-list-picture">
							<?if(!empty($arParams["LAZY_LOAD_PICTURES"]) && $arParams["LAZY_LOAD_PICTURES"] == "Y"):?>
								<img src="<?=$templateFolder?>/images/lazy.svg" alt="<?=$arElement["NAME"]?>" title="<?=$arElement["NAME"]?>" data-lazy="<?=$picture["src"]?>" class="lazy">
							<?else:?>
