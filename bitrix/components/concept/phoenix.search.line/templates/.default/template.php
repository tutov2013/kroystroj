...
if(!isset($arParams["SEARCH_CONTAINER_WIDTH"]) || !strlen($arParams["SEARCH_CONTAINER_WIDTH"]) )
    $arParams["SEARCH_CONTAINER_WIDTH"] = "full-width";
...
if($arParams["TYPE"] == "constructor")
{
    ...
    if($arParams["SEARCH_CONTAINER_WIDTH"] == "half-width")
    {
        $left_cols = "col-7";
        $right_cols = "col-5";
    }
...
<form action="<?=$PHOENIX_TEMPLATE_ARRAY["ITEMS"]["SEARCH"]["ITEMS"]["SEARCH_PAGE"]["VALUE"]?><?=(strlen($arParams["START_PAGE"]) && $arParams["START_PAGE"] != "none") ? $arParams["START_PAGE"]."/" : ""?>" class="search-form default-search-form" id="<?=$arParams["CONTAINER_ID"]?>">
    
    <div class="row align-items-center search-panel search-panel-js <?if(strlen($hint) > 0):?>hint<?endif;?>">
    
        <div class="<?=$left_cols?>">
            <div class="search-input-box">
    
                <input id="<?=$arParams["INPUT_ID"]?>" name ="q" class = "search-style search-js search-scroll-focus" type="text" value="<?=$query?>" placeholder = "<?=$placeholder?>" autocomplete="off">
        ...
        <div class="<?=$right_cols?> wrapper-right">
            ...
            <div class="row no-gutters align-items-center search-btns-box <?if(/*strlen($hint) <= 0 ||*/ strlen($query) > 0):?>active before-active<?endif;?>">
                ...
                <div class="col-lg-6 col-12 buttons">
                    ...
                    <button class = "search-btn-style mob main-color <?=$PHOENIX_TEMPLATE_ARRAY["ITEMS"]["DESIGN"]["ITEMS"]["BTN_VIEW"]['VALUE']?> d-block d-sm-none" type = "submit"></button>
            ...
            <?if(strlen($hint) > 0):?>
                <div class="hint-area hidden-sm hidden-xs paste-in-input-parent" style="display:<?=(strlen($query)) ? "none":""?>"><?=$PHOENIX_TEMPLATE_ARRAY["MESS"]["SEARCH_HINT"]?><span class="paste-in-input" id="<?=$arParams["CONTAINER_ID"]?>_hint"><?=$hint?></span></div>
            <?endif;?>
...
<script>
    ...
    <?if(strlen($query)):?>
        scrollToBlock('#<?=$arParams["CONTAINER_ID"]?>');
    <?endif;?>
