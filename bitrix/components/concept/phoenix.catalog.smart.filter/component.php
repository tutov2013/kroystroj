...
if($this->StartResultCache(false, 'v2'.($arParams["CACHE_GROUPS"]? $USER->GetGroups(): false)))
{
	...
	if(!empty($arResult["ITEMS"]))
	{
		if ($this->facetConceptPhoenix->isValid())
		{
			$this->facetConceptPhoenix->setPrices($arResult["PRICES"]);
			$this->facetConceptPhoenix->setSectionId($this->SECTION_IDS); // filter by sections
			$this->facetConceptPhoenix->setElementId($this->ELEMENT_IDS); // filter by elements
			...
			$res = $this->facetConceptPhoenix->query($arResult["FACET_FILTER"]);
			...
			while ($row = $res->fetch())
			{
				...
				if (\Bitrix\Iblock\PropertyIndex\Storage::isPropertyId($facetId))
				{
					...
					{
						$addedKey = $this->fillItemValues($arResult["ITEMS"][$PID], $this->facetConceptPhoenix->lookupDictionaryValue($row["VALUE"]), true);
						if ($addedKey)
						{
		...
		else
		{
			$arElementFilter = array(
				...
				"SUBSECTION" => $this->SECTION_IDS, // filter by sections
				"ID" => $this->ELEMENT_IDS, // filter by elements
...
{
	$this->facetConceptPhoenix->setPrices($arResult["PRICES"]);
	$this->facetConceptPhoenix->setSectionId($this->SECTION_ID);
}
			...
			foreach($arItem["VALUES"] as $key => $ar)
			{
				...
				)
				{
					if($arItem["PROPERTY_TYPE"] == "N")
					{
						...
						{
							if ($key == "MIN")
								$this->facetConceptPhoenix->addNumericPropertyFilter($PID, ">=", $_CHECK[$ar["CONTROL_NAME"]]);
							elseif ($key == "MAX")
								$this->facetConceptPhoenix->addNumericPropertyFilter($PID, "<=", $_CHECK[$ar["CONTROL_NAME"]]);
						}
					}
					elseif(isset($arItem["PRICE"]))
					{
						...
						{
							if ($key == "MIN")
								$this->facetConceptPhoenix->addPriceFilter($arResult["PRICES"][$PID]["ID"], ">=", $_CHECK[$ar["CONTROL_NAME"]]);
							elseif ($key == "MAX")
								$this->facetConceptPhoenix->addPriceFilter($arResult["PRICES"][$PID]["ID"], "<=", $_CHECK[$ar["CONTROL_NAME"]]);
						}
					}
					elseif($arItem["DISPLAY_TYPE"] == "U")
					{
						...
						{
							if ($key == "MIN")
								$this->facetConceptPhoenix->addDatetimePropertyFilter($PID, ">=", MakeTimeStamp($_CHECK[$ar["CONTROL_NAME"]], FORMAT_DATE));
							elseif ($key == "MAX")
								$this->facetConceptPhoenix->addDatetimePropertyFilter($PID, "<=", MakeTimeStamp($_CHECK[$ar["CONTROL_NAME"]], FORMAT_DATE) + 23*3600+59*60+59);
						}
					}
					elseif($_CHECK[$ar["CONTROL_NAME"]] == $ar["HTML_VALUE"])
					{
						...
						if ($arResult["FACET_FILTER"])
						{
							if ($arItem["USER_TYPE"] === "DateTime")
								$this->facetConceptPhoenix->addDatetimePropertyFilter($PID, "=", MakeTimeStamp($ar["VALUE"], FORMAT_DATE));
							else
								$this->facetConceptPhoenix->addDictionaryPropertyFilter($PID, "=", $ar["FACET_VALUE"]);
						}
					}
...
if ($_CHECK)
{
	...
	if ($arResult["FACET_FILTER"])
	{
		if (!$this->facetConceptPhoenix->isEmptyWhere())
		{
			if(!empty($arResult["ITEMS"]))
			{
				foreach ($arResult["ITEMS"] as $PID => &$arItem)
			...
			if ($arResult["CURRENCIES"])
				$this->facetConceptPhoenix->enableCurrencyConversion($this->convertCurrencyId, array_keys($arResult["CURRENCIES"]));
					...
					if(!empty($arResult["PRICES"]))
					{
						foreach($arResult["PRICES"] as $NAME => $arPrice)
						{
							...
							)
							{
								...
								if ($existCurrency)
									$currency = $this->facetConceptPhoenix->lookupDictionaryValue($currency);
...
$paramsToDelete = array("set_filter", "del_filter", "del_url", "ajax", "bxajaxid", "AJAX_CALL", "mode");
...
if(strpos($pageURL, 'arrFilter') !== false)
{
	$arTmpUrl = explode('?', $pageURL);
	$arUrl = explode('&', $arTmpUrl[1]);
	if($arUrl)
	{
		foreach($arUrl as $key => $url)
		{
			if(strpos($url, 'arrFilter') !== false)
			{
				$addParam = explode('=', $url);
				$paramsToDelete[] = $addParam[0];
			}
		}
	}
	// print_r($arUrl);
...
if(isset($_REQUEST["ajax"]) && $_REQUEST["ajax"] === "y")
{
	...
	if($this->SECTION_IDS)
		$arFilter["SECTION_ID"] = $this->SECTION_IDS;
	if($this->ELEMENT_IDS)
		$arFilter["ID"] = $this->ELEMENT_IDS;
