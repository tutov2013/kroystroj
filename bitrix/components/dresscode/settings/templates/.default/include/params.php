<?
	...
	if(!empty($template_theme)){
		$arThemeColors = !empty($template_background_name) ? $arResult["TEMPLATES"]["THEMES_COLORS"][$template_background_name]["VARIANTS"][$template_theme] : $arResult["TEMPLATES"]["THEMES_COLORS"][$template_theme];
	}
	...
	//auto available product for smart filter
	if(!empty($arResult["CURRENT_SETTINGS"]["TEMPLATE_USE_AUTO_AVAILABLE_PRODUCTS"])){
		$template_use_auto_available = $arResult["CURRENT_SETTINGS"]["TEMPLATE_USE_AUTO_AVAILABLE_PRODUCTS"];
	}

	else{
		$template_use_auto_available = "N";
	}
	...
	//auth types
	$arAuthTypes = array(
		"PASS" => GetMessage("SETTINGS_AUTH_TYPE_PASS"),
		"SMS" => GetMessage("SETTINGS_AUTH_TYPE_SMS_CODE"),
	);

	//select auth type
	if(!empty($arResult["CURRENT_SETTINGS"]["TEMPLATE_AUTH_TYPE"])){
		$template_auth_format = $arResult["CURRENT_SETTINGS"]["TEMPLATE_AUTH_TYPE"];
	}

	else{
		$template_auth_format = key($arAuthTypes);
	}

	//masked input
	if(!empty($arResult["CURRENT_SETTINGS"]["TEMPLATE_USE_MASKED_INPUT"])){
		$template_use_masked_input = $arResult["CURRENT_SETTINGS"]["TEMPLATE_USE_MASKED_INPUT"];
	}
	...
	//masked types
	$arMaskedFormates = array(
		"+7 (999) 999-99-99" => GetMessage("SETTINGS_MASKED_INPUT_COUNTRY_RUSSIA"),
		"+380 (999) 999-99-99" => GetMessage("SETTINGS_MASKED_INPUT_COUNTRY_UKRAINE"),
		"+375 (999) 999-99-99" => GetMessage("SETTINGS_MASKED_INPUT_COUNTRY_BIELORRUSIA"),
	);

	//select masked format
	if(!empty($arResult["CURRENT_SETTINGS"]["TEMPLATE_MASKED_INPUT_FORMAT"])){
		$template_masked_input_format = $arResult["CURRENT_SETTINGS"]["TEMPLATE_MASKED_INPUT_FORMAT"];
	}

	else{
		$template_masked_input_format = key($arMaskedFormates);
	}
