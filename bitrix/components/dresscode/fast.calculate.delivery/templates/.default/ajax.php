...
if(!empty($_REQUEST["act"])){
	...
	if($_REQUEST["act"] == "getCalculatedItems"){
		...
		if(!empty($_REQUEST["PARAMS"]["PRODUCT_ID"])){
			...
			//get delivery data
			$arResult["DELIVERY_ITEMS"] = \DigitalWeb\CalculateDelivery::getCalculatedItems($_REQUEST["PARAMS"]);
			$arResult["MEASURE_RATIO"] = \DigitalWeb\Basket::getMeasureRatio($_REQUEST["PARAMS"]["PRODUCT_ID"]);
