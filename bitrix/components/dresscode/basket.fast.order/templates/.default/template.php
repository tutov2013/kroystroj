		...
		<div class="fast-basket-container">
			...
			<form action="#" name="fast-basket-form" method="get" class="fast-basket-container__form" id="bind__fast-basket-form">
				<div class="basket-container-form__field">
					<input type="text" name="basket-form-name" class="container-form__name" placeholder="<?=GetMessage("FAST_BASKET_NAME_PLACEHOLDER")?>"<?if(!empty($arResult["USER_NAME"])):?> value="<?=$arResult["USER_NAME"]?>"<?endif;?>>
				</div>
				<div class="basket-container-form__field">
					<input type="text" name="basket-form-telephone" class="container-form__telephone" data-required="yes" placeholder="<?=GetMessage("FAST_BASKET_TELEPHONE_PLACEHOLDER")?>"<?if(!empty($arResult["USER_PHONE"])):?> value="<?=$arResult["USER_PHONE"]?>"<?endif;?>>
					<?if($arParams["USE_MASKED"] == "Y" && !empty($arParams["MASKED_FORMAT"])):?>
						<script>
							$(function(){
								$(".container-form__telephone").mask("<?=$arParams["MASKED_FORMAT"]?>");
							});
						</script>
					<?endif;?>
					<?if(!empty($arParams["SEND_SMS_MESSAGE"]) && $arParams["SEND_SMS_MESSAGE"] == "Y"):?>
						<input type="hidden" name="basket-form-sms-send" value="<?=$arParams["SEND_SMS_MESSAGE"]?>" class="container-form__send-sms">
					<?endif;?>
				</div>
				<div class="basket-container-form__field">
